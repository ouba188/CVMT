...
@TRANSFORMS.register_module()
class RandomCrop_Dual(BaseTransform):
    """Random crop the image & bboxes & masks.
    ...
    def _crop_data(self, results: dict, crop_size: Tuple[int, int],
        ...
        img2 = results['img2']
        results['img'] = img1
        margin_h = max(img1.shape[0] - crop_size[0], 0)
        margin_w = max(img1.shape[1] - crop_size[1], 0)
        ...
        # crop the image
        img1 = img1[crop_y1:crop_y2, crop_x1:crop_x2, ...]
        img2 = img2[crop_y1:crop_y2, crop_x1:crop_x2, ...]
    ...
    def transform(self, results: dict) -> Union[dict, None]:
        ...
        Returns:
            results (Union[dict, None]): Randomly cropped results, 'img_shape'
                ...
                be returned when there is no valid bbox after cropping.
        """
        image_size = results['img1'].shape[:2]
        crop_size = self._get_crop_size(image_size)
        results = self._crop_data(results, crop_size, self.allow_negative_crop)
        return results
