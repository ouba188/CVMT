...
class HieAssigner(BaseAssigner):
    ...
    def assign(self, bboxes, gt_bboxes, gt_bboxes_ignore=None, gt_labels=None):
        ...
        assign_on_cpu = True if (self.gpu_assign_thr > 0) and (
            gt_bboxes['bboxes'].shape[0] > self.gpu_assign_thr) else False
        ...
        overlaps = self.iou_calculator(gt_bboxes, bboxes["priors"], mode=self.assign_metric)
        bboxes2 = self.anchor_rescale(bboxes["priors"], self.ratio)
        overlaps2 = self.iou_calculator(gt_bboxes, bboxes2, mode=self.assign_metric)



        if (self.ignore_iof_thr > 0 and gt_bboxes_ignore is not None
                and gt_bboxes_ignore.numel() > 0 and bboxes["priors"].numel() > 0):
            if self.ignore_wrt_candidates:
                ignore_overlaps = self.iou_calculator(
                    bboxes["priors"], gt_bboxes_ignore, mode='iof')
                ignore_max_overlaps, _ = ignore_overlaps.max(dim=1)
            else:
                ignore_overlaps = self.iou_calculator(
                    gt_bboxes_ignore, bboxes["priors"], mode='iof')
        ...
        if self.inside==True:
            num_anchors = bboxes["priors"].size(0)
            num_gts = gt_bboxes.size(0)

            anchor_cx = (bboxes["priors"][...,0]+bboxes["priors"][...,2])/2
            anchor_cy = (bboxes["priors"][...,1]+bboxes["priors"][...,3])/2
