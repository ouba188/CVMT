...
def bbox_center_distance(bboxes: Tensor, priors: Tensor) -> Tensor:
    ...
    priors_points = priors_points.to(device)
...
class ATSSAssigner(BaseAssigner):
    ...
    def assign(
        ...
        gt_bboxes= gt_bboxes.to(d)
        ep_priors_cx = ep_priors_cx.to(d)
        ep_priors_cy = ep_priors_cy.to(d)
        ...
        if pos_inds.numel() > 0:
            gt_labels = gt_labels.to(d)
            assigned_gt_inds = assigned_gt_inds.to(d)
